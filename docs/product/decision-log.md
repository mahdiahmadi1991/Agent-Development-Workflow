...
| D-091 | Scenario matrix coverage must be tracked in a canonical traceability map and validated in CI/governance gates. | Accepted | Canonical map: `docs/product/scenario-coverage-map.yaml`; validator: `scripts/validate-scenario-coverage.sh`. |
| D-092 | Pre-install transparency gate must expose selection explainability before apply (compact summary + explicit details path). | Accepted | Implemented in install transparency flow and reflected in post-install explainability block. |
| D-093 | Topic catalog is intentionally reset to empty placeholders until real, researched topic content is authored. | Accepted | Keep taxonomy directories with `.gitkeep`; index remains valid with `topics: []`. |
