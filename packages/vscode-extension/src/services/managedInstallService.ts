...
export interface ManagedInstallResult {
  ...
  appliedArtifactsReportPath?: string;
  appliedFiles: string[];
  skippedFiles: string[];
...
function buildAppliedArtifactsReportContent(input: {
  mode: ManagedApplyMode;
  bundleId: string;
  bundleVersion: string;
  extensionVersion: string;
  ...
  const renderList = (title: string, values: string[]): string[] => {
    if (values.length === 0) {
      return [title, "- (none)", ""];
    }
    ...
    return [
      title,
      ...values.slice().sort((left, right) => left.localeCompare(right)).map((value) => `- \`${value}\``),
      ""
    ];
  };

  return [
    "# Applied Artifacts Report",
    "",
    "This runtime report is generated by the Codex Onboarding extension after successful install/repair apply.",
    "",
    ...
    "",
    ...renderList("## Files Applied In This Operation", input.appliedFiles),
    ...renderList("## Files Skipped In This Operation", input.skippedFiles),
    ...renderList("## Recovered Tracked Files", input.recoveredTrackedFiles),
    ...renderList("## Removed Stale Managed Files", input.removedStaleFiles),
    ...renderList(
      "## Current Managed File Inventory (From state.json)",
      input.managedFiles.map((item) => item.relative_path)
    )
  ].join("\n");
}

async function writeAppliedArtifactsReport(
  targetRootPath: string,
  ...
  logger.log("debug", "file_applied", {
    ...
    reason: "applied_artifacts_report_written"
  });
...
export async function applyManagedInstall(
  ...
  const appliedArtifactsReportPath = await writeAppliedArtifactsReport(
    input.targetRootPath,
    buildAppliedArtifactsReportContent({
      mode,
